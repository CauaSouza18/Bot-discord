...
client.on('interactionCreate', async (interaction) => {
  ...
  const CARGO_RELATORIO_ID = '1372769455393734656'; // ← Coloque aqui o ID do cargo que pode ver o relatório
        ...
        timersMutados[newState.id] = setTimeout(() => {
          ...
          if (newState.channel?.parentId === CATEGORIA_MONITORADA && newState.mute && newState.deaf) {
            ...
            if (canal) {
              ...
              canal.send(`❌ <@${newState.id}> foi desconectado por ficar 5 minutos mutado e sem ouvir na categoria de patrulhamento.`);
